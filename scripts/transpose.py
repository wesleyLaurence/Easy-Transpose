...
class Transpose:
    ...
    def __init__(self):
        ...
        file_path = Path(str(pathlib.Path(__file__).parent.absolute())+'/'+'music_notes.csv')
        self.Music_Pitch_System = pd.read_csv(file_path)
        self.all_notes = None
    ...
    #print("Transposed Pitch ID: ",transposed_pitch_ID)
        ...
        if self.database_connect == True:
            db = SQL_Kit(self.userID, self.password, 'easytranspose')
    ...
    def select_table(self, table):
        ...
        d = Dashboard(self.userID, self.password, self.database)
        data = d.select_table(table)
